...
echo "Generating a rootCA.csr..."
openssl req -passin pass:pass -new -key rootCA.key -out rootCA.csr -subj $cert_subject
...
openssl rsa -passin pass:pass -in rootCA.key.orig -out rootCA.key
...
openssl req -passin pass:pass -new -key client.key -out client.csr -subj $cert_subject

cp client.key client.key.orig

openssl rsa -passin pass:pass -in client.key.orig -out client.key
...
openssl req -passin pass:pass -new -key server.key -out server.csr -subj $cert_subject

cp server.key server.key.orig

openssl rsa -passin pass:pass -in server.key.orig -out server.key
...
openssl req -passin pass:pass -new -key server_bkup.key -out server_bkup.csr -subj $cert_subject

cp server_bkup.key server_bkup.key.orig

openssl rsa -passin pass:pass -in server_bkup.key.orig -out server_bkup.key
